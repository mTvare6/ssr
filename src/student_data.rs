...
fn get_student_response() -> Result<String, Box<dyn std::error::Error>> {
    ...
    let key_body = ApiKeyBody {
        key: String::from("BFXDWTSqEFOXbk5kzCYsqV50vL4YjUQywDUfcE5wy0cD01SsJ7nLFduSzjE4Or3W"),
    };
    ...
    let response = ureq::post(format!(
        "https://ap-south-1.aws.data.mongodb-api.com/app/{}/endpoint/data/v1/action/find",
        api_id
    ))
    .header(
        "Authorization",
        format!("Bearer {}", recv_body.access_token),
    )
